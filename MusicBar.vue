      ...
      $route: function(route) {
        ...
        for (let i in paths) {
          ...
          if (match(i, route.path).matches) {
            console.log(1234);
            playList =
              MUSICBAR_OPTIONS[i].playList || MUSICBAR_OPTIONS.playList || [];
            platform =
              MUSICBAR_OPTIONS[i].platform || MUSICBAR_OPTIONS.platform || [];
            firstClickPlay =
              MUSICBAR_OPTIONS[i].firstClickPlay ||
              MUSICBAR_OPTIONS.firstClickPlay ||
              true;
            background =
              MUSICBAR_OPTIONS[i].background ||
              MUSICBAR_OPTIONS.background ||
              "";
            timeOut =
              MUSICBAR_OPTIONS[i].timeOut || MUSICBAR_OPTIONS.timeOut || 2000;
            frameColor =
              MUSICBAR_OPTIONS[i].frameColor ||
              MUSICBAR_OPTIONS.frameColor ||
              "";
      ...
      AudioInit_163: function(e) {
        ...
        if (e.playListIDs) {
          e.playListIDs.forEach((id) => {
            fetch(
              "https://netease-cloud-music-api-two-rust.vercel.app/playlist/track/all?id=" +
                id.toString()
            )
              .then((response) => response.json())
              .then((data) => {
                ...
                data.songs.forEach((e, i) => {
                  fetch(
                    "https://netease-cloud-music-api-two-rust.vercel.app/song/url/?id=" +
                      e.id.toString()
                  )
                    .then((response) => response.json())
                    .then((data) => {
